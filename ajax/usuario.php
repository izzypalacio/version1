    ...
    case "guardaryeditar":
      ...
      if ($password1 == $password2) {
        ...
        if (empty($_POST["id_usuario"])) {
          ...
          /*si coincide password1 y password2 entonces verificamos si existe la dui y correo en la base de datos, si ya existe un registro con la dui o correo entonces no se registra el usuario*/

          $datos = $usuarios->get_dui_correo_del_usuario($_POST["dui"], $_POST["email"]);

          if (is_array($datos) == true and count($datos) == 0) {

            //no existe el usuario por lo tanto hacemos el registros

            $usuarios->registrar_usuario($nombre, $apellido, $dui, $telefono, $email, $direccion, $cargo, $usuario, $password1, $password2, $estado, $permisos);

            $messages[] = "El usuario se registró correctamente";

            /*si ya exista el correo y la dui entonces aparece el mensaje*/
          } else {

            $errors[] = "El dui o el correo ya existe";
          }
          ...
          $usuarios->editar_usuario($id_usuario, $nombre, $apellido, $dui, $telefono, $email, $direccion, $cargo, $usuario, $password1, $password2, $estado, $permisos);
  ...
  ?>
    ...
    case "mostrar":
      ...
      if (is_array($usuario_compras) == true and count($usuario_compras) == 0 and is_array($usuario_ventas) == true and count($usuario_ventas) == 0) {
        ...
        //si el id_usuario tiene relacion con la tabla compras y tabla ventas entonces se deshabilita el nombre, apellido y dui


        foreach ($datos as $row) {

          $output["dui_relacion"] = $row["dui"];
          $output["nombre"] = $row["nombres"];
          $output["apellido"] = $row["apellidos"];
          $output["cargo"] = $row["cargo"];
          $output["usuario"] = $row["usuario"];
    ...
    case "eliminar_usuario":
      ...
      $Dproducto = $Dproducto->get_Dproducto_por_id_usuario($_POST["id_usuario"]);

      $Batida = $Batida->get_Batida_por_id_usuario($_POST["id_usuario"]);

      $cif = $cif->get_cif_por_id_usuario($_POST["id_usuario"]);

      $Mprima = $Mprima->get_Mprima_por_id_usuario($_POST["id_usuario"]);

      $Cfabricacion = $Cfabricacion->get_Cfabricacion_por_id_usuario($_POST["id_usuario"]);
      ...
      if (
        ...
        is_array($Dproducto) == true and count($Dproducto) > 0 or
        is_array($Mprima) == true and count($Mprima) > 0 or
        is_array($Cfabricacion) == true and count($Cfabricacion) > 0 or
        is_array($Batida) == true and count($Batida) > 0 or
        is_array($cif) == true and count($cif) > 0
      ) {
